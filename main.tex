...
Aerosol in the stratosphere has been monitored via remote sensing applications throughout the last few decades. Solar occultation has been the method used the longest to monitor aerosol in the stratosphere and has built up a reliable, accuracy and long term database of background and volcanic aerosol extinctions. Notable occultation instruments include SAM II \citep{McCormick1979}, SAGE II \citep{McCormick1987}, and SAGE III \citep{Thomason2003} and their life cycles gathered measurements over a time period from 1978 to 2005. The occultation method was used due to its ability to self-calibrate each measurement by directly viewing the sun outside of the area of interest giving stable profiles and the method also directly measures the optical depth allowing for simple conversion to extinction without the the need for particle size knowledge.
...
Next was an experiment to determine the stray light characteristics of the overall system. Stray light removal has always been difficult in atmospheric instrumentation due to the difficulty in accurately discerning the signal in regards to the stray light contamination. Furthermore, ALI's optical system has the further addition of unwanted light internal to the instrument because of the rejection of one of the polarizations due to the nature of the AOTF. The signal enters the optical system unpolarized, but only one polarization can have a consistent output angle from the AOTF. The entering light is passed through a linear polarizer with an extinction ratio of at least 100,000:1 to remove the unwanted polarization however a small percentage is not absorbed. Furthermore, a second linear polarizer is used after the AOTF to reject all of the unwanted radiance that did not meet the Bragg criteria and once again a small percentage of this radiance is not absorbed. However, the AOTF requires a RF signal to be able to diffract a specific wavelength, using this characteristic another experiment was devised to determine the stray light of the system. A 250~W quartz-tungsten light source passed through a dispersing screen and then passed into the entrance aperture of ALI with even illumination. Using variety of exposure times ranging from 0.1~s to 60~s and wavelengths from 650 to 950~nm with exposures every 25~nm the incoming source was imaged twice for each unique combination. One image recorded with the AOTF in its off state, with no driving RF wave, and one with the ATOF in its on state, with the RF wave. Once all the images were acquired the DC offset was removed and the counts were divided by the exposure times to give counts per second. Essentially, the image with the AOTF off only contains stray light in the system and the AOTF on image contains the stray light combined with the even signal from the light source. By subtracting the AOTF on image from the AOTF off the resulting image just contains the signal and effects from the stray light from the system have been removed. The resulting image contains a bright centre with the a fallout in brightness which, is due to the known vignetting of the system with is caused by the aperture of the AOTF, with no significant bright or dark spots in the image. During the mission the AOTF on minus AOTF off method is used to remove the stray light in the final images.

Lastly, the vignetting must be normalized across the whole field of view to be be able to determine the radiances from mission. The resulting images from the stray light removal experiment were used. For each wavelength, coefficients were determined to convert all pixels to a unified value, the average counts per second of the centre 10 by 10 pixels, for each exposure time. The mean of the coefficients for each wavelength were used. Furthermore, the images needed to be flat fielded over wavelength, since no absolute calibration was performed on ALI the flat field coefficient were normalized to the 750~nm value. Once a image hs gone through the omplete calibration process, from DC offet to stray light removal to flat fielding calibration, will be known as relative radiance since the radiance are normalized relative to the 750~nm laboratory measured counts.

During the calibration process software was designed for the ALI system to be used for the balloon campaign. ALI ran off of a Debian Linux operating system with threaded C++ based software to control the hardware and science operations that would start up with system power on. The onboard system communicated to a ground based station where it to send data, including images and house keepping information, and received commands. Before the mission, a complete system test was preformed to verify successful use of all hardware and software modules before the Timmins campaign, which was successful. ALI was then prepaired for transport and sent to Timmins, Ontario for the balloon launch.
...
After the successful recovery of ALI the data was converted into relative radiances using the calibration methods discussed in \autoref{sec:Calibration} here the DC offset is remove, followed by the stray light removal, then the application of the flat fielding coefficients. An example of a raw image and a calibrated image ca be seen in \autoref{fig:BeforeAfterImages} which is image number 212, a 750~nm image. The before image is the raw data directly after the mission with no calibrations preformed and the after image in the bottom panel is the same image after the all the calibrations have been applied.
