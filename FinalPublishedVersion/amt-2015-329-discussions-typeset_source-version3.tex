...
\begin{abstract}
  ...
  indicate that the radiance measurements are of high quality, and we
...
and particle size, the observations are essentially always limited to
some degree. In-situ balloon observations continue to be used and have
...
aerosol has been in use since the 1970's, beginning with limb sounding
...
comparing the retrieved extinction profiles to other microphysical
parameters (Russell and McCormick, 1989; Kovilakam et~al.,
...
straight forward retrieval of aerosol extinction coefficient (Damadeo
et~al., 2013). Although the currently operational MAESTRO and
ACE-Imager instruments on SciSat (McElroy et~al., 2007; Gilbert
et~al., 2007) have had some success producing stratospheric aerosol
...
with SAGE III in 2006. However, a~manifestation of SAGE III is planned
...
agree relatively well with SAGE II and SAGE III occultation
...
vertical resolution (Ernst et~al., 2012; von Savigny et~al., 2015)
...
studied extensively and somewhat controversially (Bourassa et~al., 2012c,
...
2012), which is a~Belgian instrument concept from at the Belgian
...
the acousto wave. The parameters $\lambda$, $F$ and $\nu$ are the
...
aperture imaging quality ATOF was acquired from Brimrose of America
...
modes respectively (Uchida, 1971). The acousto-optic diffraction angle
is not constant angle with wavelength, so in order to achieve an
...
response of a~standard and readily available CCD detector. We also
attempted to pay careful attention to stray light reduction including
both internal scatter and out-of-field signal.

The use of the AOTF essentially limits the optical design to two
possible basic layouts: the telecentric or the telescopic system. This
...
\end{align}
where $\delta$ is the displacement from the original path. However, it
turns out that this wavelength dependent change is negligible for the
current ALI design.
...
through the system.  The diffracted extraordinary signal compresses at
...
of less than 1\,{\%} change across the entire wavelength range. The
...
final image. Dekemper et~al.~(2012) reports that the visible channel of
...
had a~spectral resolution of 1.2\,\unit{nm}, which is much less than
the factory specified resolution of the ATOF. Images were taken at
...
once with the ATOF in its on state, with the acoustic wave applied
...
immediately obtained with the same exposure time.
...
removal, and flat fielding correction error terms. The spectra displays the
...
approximately 30\,\unit{km} tangent height. The second term in Eq.~(7) uses
...
this line corresponds to the Angstrom exponent. This is then used to
...
the process is iterated until the Angstrom exponent, corresponding to
...
Fig.~12. Note the log scale. After the retrieval, the difference
...
error and neglects any model and atmospheric state errors. The green
curve is the average 750\,\unit{nm} aerosol extinction profiles of the
...
total retrieval uncertainty below 20\,\unit{km}. It is encouraging,
...
15\,\unit{km}.  However, the OSIRIS and ALI extinctions do not agree
within error between 20 to 25\,\unit{km}. Aerosol is notoriously
...
(Bourassa et~al., 2012b; Rieger et~al., 2015) so the disagreement
between OSIRIS and ALI from 20 to 25\,\unit{km} found here is somewhat
...
10\,{\%}. There are also several possible systematic errors not
accounted for in the inversion including the choice of retrieval
altitude ranges, particle size composition and distributions, stray
light, and the high altitude aerosol load. This is also the first
polarized limb scatter retrieval to our knowledge and so there may be
...
mode radius of 0.077\,\unit{{\mu}m}. In comparison to typical levels
...
within an expected range, although there is a~relatively large error
...
\caption{A~calibration image after stray light removal has been
  ...
  horizontal field of view is on the right side is lost due to strong
...
\caption{\textbf{(a)} The GPS data from ALI during the Nimbus 7
  ...
  mission. Important landmarks are noted on the image.  The end of
  ...
  cyan vertical line and first light measured by ALI is occurs at the
...
\caption{Left is the retrieved aerosol extinction profiles from the
  ...
  ALI aerosol extinction in blue with its error represented by the
  shading compared to the 750\,\unit{nm} extinction measured by OSIRIS
  in green.}
