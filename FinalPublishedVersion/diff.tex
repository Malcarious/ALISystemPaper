...
%DIF DEL amt-2015-329-discussions-typeset_source-version3.tex   Fri Jan 15 11:02:17 2016
%DIF ADD amt-2015-329-discussions-typeset_source-version4.tex   Mon Mar 14 11:40:21 2016
...
and particle size, \DIFdelbegin \DIFdel{the observations are essentially always limited to
some degree}\DIFdelend \DIFaddbegin \DIFadd{no single measurement technique can determine the full range of aerosol properties unambiguously}\DIFaddend . In-situ balloon observations continue to be used and have
...
aerosol has been in use since the \DIFdelbegin \DIFdel{1970's}\DIFdelend \DIFaddbegin \DIFadd{1970s}\DIFaddend , beginning with limb sounding
...
\DIFdelbegin \DIFdel{comparing the retrieved extinction profiles to other microphysical
parameters }\DIFdelend \DIFaddbegin \DIFadd{determining microphysical
parameters and comparison between instruments can be challenging. }\DIFaddend (Russell and McCormick, 1989; Kovilakam et~al.,
...
\DIFdelbegin \DIFdel{straight forward }\DIFdelend \DIFaddbegin \DIFadd{straightforward }\DIFaddend retrieval of aerosol extinction coefficient (Damadeo
et~al., 2013). \DIFdelbegin \DIFdel{Although the }\DIFdelend \DIFaddbegin \DIFadd{The SAGE III mission came to an end in 2006 and the occultation measurements have continued from the }\DIFaddend currently operational MAESTRO and
ACE-Imager instruments on SciSat (McElroy et~al., 2007; Gilbert
et~al., 2007) \DIFaddbegin \DIFadd{and }\DIFaddend have had some success producing stratospheric aerosol
extinction products (Vanhellemont et~al., 2008; Sioris et~al., 2010)\DIFdelbegin \DIFdel{,
the era of solar occultation measurements essentially came to an end
with SAGE III in 2006. However, }\DIFdelend \DIFaddbegin \DIFadd{.
Furthermore, }\DIFaddend a~manifestation of SAGE III is planned
...
studied extensively and \DIFdelbegin \DIFdel{somewhat controversially }\DIFdelend \DIFaddbegin \DIFadd{the conclusions are somewhat controversial }\DIFaddend (Bourassa et~al., 2012c,
...
operating beyond its design lifetime. \DIFaddbegin \DIFadd{The lidar instrument Cloud Aerosol Transport System (CATS) (Chuang et al., 2013) has been placed on the international space station in 2015 for an expected mission lifetime of three years.
}\DIFaddend 
...
of the AOTF \DIFdelbegin \DIFdel{, which is typically between 3--6\,}%DIFDELCMD < \unit{nm} %%%
...
aperture imaging quality \DIFdelbegin \DIFdel{ATOF }\DIFdelend \DIFaddbegin \DIFadd{AOTF }\DIFaddend was acquired from Brimrose of America
...
modes respectively (Uchida, 1971). The acousto-optic diffraction angle \DIFdelbegin \DIFdel{is not constant angle with wavelength }\DIFdelend \DIFaddbegin \DIFadd{varies as the filtered wavelength is changed}\DIFaddend , so in order to achieve an
...
input optical axis of the device \DIFaddbegin \DIFadd{with a minimum separation angle of 6.4}{\degree} \DIFadd{between the zeroth and first order}\DIFaddend . The ordinary light beam also
...
response of a~standard and readily available CCD detector. We \DIFdelbegin \DIFdel{also
attempted to pay }\DIFdelend \DIFaddbegin \DIFadd{paid }\DIFaddend careful attention to stray light reduction including
both internal scatter and out-of-field signal.

The use of the AOTF essentially limits the optical design to two
possible basic layouts: the telecentric or the telescopic system. \DIFaddbegin \DIFadd{The telecentric system uses a layout that removes perspective from the image and object plane by creating a condition that requires the chief ray to be parallel to the optical axis in both object and image space. The telescopic system uses a simple two lens afocal system to resize and collimate the incoming rays of light into the AOTF. }\DIFaddend This
...
\end{align}
where $\delta$ is the displacement from the original path \DIFaddbegin \DIFadd{and $t$ is the
thickness of the crystal}\DIFaddend . However, \DIFdelbegin \DIFdel{it
turns out that }\DIFdelend this wavelength
dependent change is \DIFdelbegin \DIFdel{negligible }\DIFdelend \DIFaddbegin \DIFadd{less than a micrometer }\DIFaddend for the current ALI design \DIFaddbegin \DIFadd{and is considered negligible}\DIFaddend .
...
error in the central wavelength (see Fig.~6b). \DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{Even with considering the temperature change, the AOTF would experience a maximum wavelength drift of 2.5\,nm during the the mission which is acceptable for the slowly varying broadband scattering cross section of aerosol. Furthermore, it }\DIFaddend should be noted that
...
\DIFdelbegin \DIFdel{The same set of experimental data was also used }\DIFdelend \DIFaddbegin \DIFadd{An experiment was performed on several wavelengths }\DIFaddend to determine the \DIFaddbegin \DIFadd{RF power that yielded the highest throughput through the AOTF using an collimated light source. For the AOTF in ALI, the maximum throughput occurred when the RF power was at the limit of the AOTF which was 2\,W. Following, the }\DIFaddend diffraction efficiency of the AOTF \DIFdelbegin \DIFdel{, along with an additional
measurement }\DIFdelend \DIFaddbegin \DIFadd{was determined by using two sets measurements. The first is the experimental data used to perform the wavelength calibration, the second is measurements }\DIFaddend of the intensity of the incident collimated light beam. \DIFdelbegin \DIFdel{This was simply acquired by removing the AOTF from the same
experimental setup. }\DIFdelend \DIFaddbegin \DIFadd{The light in both experiments was linearly polarized and aligned with the polarization axis of the AOTF and for the second set the AOTF was simply removed from the optical chain. It should be noted that the attenuation of the AOTF crystal itself was not determined independently and will be combined with the diffraction efficiency. We are more concerned about signal throughput of the device so the combination of the effects is acceptable. }\DIFaddend The incident light source was then measured with the \DIFaddbegin \DIFadd{same }\DIFaddend iHR320 spectrometer and Synapse CCD. By taking the ratio of the intensity at the diffracted wavelength to the incident intensity the diffraction efficiency was determined. It was found to vary between
...
once with the \DIFdelbegin \DIFdel{ATOF }\DIFdelend \DIFaddbegin \DIFadd{AOTF }\DIFaddend in its on state, with the acoustic wave applied
...
immediately obtained with the same exposure time. \DIFaddbegin \DIFadd{For the calibration images an average stray light to signal ratio of 2.5$\cdot10^{-2}$ was noted.
}\DIFaddend 
...
value across the image. The \DIFdelbegin \DIFdel{resulting images from the }\DIFdelend \DIFaddbegin \DIFadd{experiment was set up using a 250\,W halogen bulb that was collimated and passed through a diffusing plate to yield a consistent even output for the source. The entrance aperture of ALI was placed 100\,mm from the diffusing plate and was completely illuminated. The diffusing plate was imaged at a variety of wavelengths (from 600 to 1000\,nm) and exposure times (ranging from 0.1 seconds to 2 minutes). Images from the }\DIFaddend diffuse source
...
tests. \DIFaddbegin \DIFadd{For the high altitudes in the range of 27 to 30\,km the expected ratio of signal to stray light was estimated to be between 2-3 but for the campaign the ratio of signal to stray light for some regions dropped down slightly below one. }\DIFaddend This may be due to contamination from scattering from a~baffle vein or
...
\begin{align}
  ...
  \right)-\log {\left(\DIFdelbegin \DIFdel{\frac{\vec{I}_{\text{v,
            rayleigh}}\left(\vec{z},\lambda \right)}{I_{\text{v,
            rayleigh}}\left(z_{\text{ref}},\lambda \right)}
    }\DIFdelend \DIFaddbegin \DIFadd{\frac{\vec{I}_{\text{v,
...
approximately 30\,\unit{km} tangent height \DIFaddbegin \DIFadd{and typical values for $z_{ref}$ were between 27 and 30\,}\unit{km}\DIFaddend . The second term in Eq.~(7) uses
...
\DIFadd{This results in relative error in the measurement vectors of approximately 10-15\% for the cross terms and and 25-35\% for the diagonal terms. Propagating this uncertainty though the retrieval method results in large uncertainty on the retrieved aerosol extinction around 40-70\% dependent on the altitude. Reducing the error in the relative calibration would greatly improve the overall uncertainty in the ALI retrievals since it contributes the current largest factor to the calibrated radiances.
}

\DIFaddend The \DIFdelbegin \DIFdel{green
}\DIFdelend \DIFaddbegin \DIFadd{750\,}\unit{nm} \DIFadd{aerosol extinction determined by ALI is compared to the retrieved aerosol extinctions by OSIRIS. In the right panel of Fig.~12 the red }\DIFaddend curve is the average 750\,\unit{nm} aerosol extinction \DIFdelbegin \DIFdel{profiles }\DIFdelend \DIFaddbegin \DIFadd{profile }\DIFaddend of the
same five coincident OSIRIS scans used for the ozone profile. The
retrieved extinction profiles from ALI and OSIRIS are within \DIFdelbegin \DIFdel{with }\DIFdelend the
total retrieval uncertainty\DIFdelbegin \DIFdel{below 20\,}%DIFDELCMD < \unit{km}%%%
\DIFdelend . It is encouraging \DIFdelbegin \DIFdel{,
however, }\DIFdelend that the instruments follow the same overall profile shape
including the stratospheric layer and the steep increase below
15\,\unit{km}. \DIFdelbegin \DIFdel{However, the OSIRIS and ALI extinctions do not agree
within error between 20 to 25\,}%DIFDELCMD < \unit{km}%%%
\DIFdel{. }\DIFdelend Aerosol is notoriously
difficult to validate in remote sensing with various technique and
instrument geometries, and \DIFdelbegin \DIFdel{yet }\DIFdelend the SAGE II, SAGE III and OSIRIS
differences are generally below 20--30\,{\%} up to 30\,\unit{km}
(Bourassa et~al., 2012b; Rieger et~al., 2015)\DIFdelbegin \DIFdel{so the disagreement
between OSIRIS and ALI from 20 to 25\,}%DIFDELCMD < \unit{km} %%%
\DIFdel{found here is somewhat
puzzling}\DIFdelend . However, given the retrieved uncertainty, the OSIRIS profile
is only outside the upper error bound of ALI by less than
10\,{\%} \DIFaddbegin \DIFadd{between 20 and 25\,}\unit{km}\DIFaddend . There are also several possible systematic errors not
accounted for in the inversion including the choice of retrieval
altitude ranges, particle size \DIFdelbegin \DIFdel{composition and  distributions}\DIFdelend \DIFaddbegin \DIFadd{distribution and  particle composition}\DIFaddend , stray
light, and the high altitude aerosol load. \DIFdelbegin \DIFdel{This is also the first
polarized limb scatter retrieval to our knowledge and so }\DIFdelend \DIFaddbegin \DIFadd{The solar scattering angle for a measurement can also have an effect on the retrieved profile due sensitivity to the scattering cross sections from the particle size distributions. For the ALI image the solar scattering angle is 98\,degrees and for the five OSIRIS scans they are 77, 89, 90, 91, 92, 93\,degrees. With the exception of the forward scatter angles of 77 and 89\,degrees from OSIRIS, the scattering angles between OSIRIS and ALI are similar and should not cause a large effect on the retrieved profiles. Furthermore, }\DIFaddend there may be
...
mode radius of 0.077\,\unit{{\mu}m} \DIFaddbegin \DIFadd{or Angstr\"{o}m coefficient of 2.7}\DIFaddend . In comparison to typical levels
...
within an expected range \DIFaddbegin \DIFadd{(Angstr\"{o}m coefficient of 2.1-3.4)}\DIFaddend , although there is a~relatively large error
...
\DIFaddbegin \bibitem{67}
\DIFadd{Chuang,~T., Burns,~P., Walters,~E.~B., Wysocki,~T., Deely,~T., Losse,~A., Le,~K.,
...
Technology and Devices, 85990N. doi:}\href{http://dx.doi.org/10.1117/12.2005545}{10.1117/12.2005545}\DIFadd{, 2013.
}

\DIFaddend \bibitem{12}
...
\DIFdelbegin %DIFDELCMD < \bibitem{19}
%DIFDELCMD < %%%
...
\DIFaddbegin \bibitem{66}
\DIFadd{Taha,~G., Rault,~D.~F., Loughman,~R.~P., Bourassa,~A.~E., and von Savigny,~C.:
SCIAMACHY stratospheric aerosol extinction profile retrieval using the OMPS/LP
algorithm, Atmos. Meas. Tech., 4, 547-556, }\doi{10.5194/amt-4-547-2011}\DIFadd{, 2011.
}

\DIFaddend \bibitem{51}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=110mm]{amt-2015-329-discussions-f06.pdf}
%DIFDELCMD < %%%
...
\caption{A~calibration image after stray light removal has been
  ...
  horizontal field of view \DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL on the right side is lost due to strong
...
\caption{\textbf{(a)} The GPS data from ALI during the Nimbus 7
  ...
  mission \DIFaddbeginFL \DIFaddFL{and the blue, green, and red colours represent speeds of approximately
  10, 70, and 140~km/h}\DIFaddendFL . Important landmarks are noted on the image.  The end of
  ...
  cyan vertical line and first light measured by ALI is \DIFdelbeginFL \DIFdelFL{occurs at }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{represented by }\DIFaddendFL the
...
\caption{Averaged ALI relative radiance vectors from 12 of the 13
  ...
  than 90{\degree}. \DIFaddbeginFL \DIFaddFL{The separation between each consecutive radiance vector at each wavelength is approximately 2 degrees in solar zenith angle. }\DIFaddendFL }
...
\caption{Left is the retrieved aerosol extinction profiles from the
  ...
  ALI aerosol extinction in blue \DIFdelbeginFL \DIFdelFL{with its error represented by the
  shading }\DIFdelendFL compared to the 750\,\unit{nm} extinction measured by OSIRIS
  in \DIFdelbeginFL \DIFdelFL{green}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{red with its error represented by the respective
  shading}\DIFaddendFL .}
